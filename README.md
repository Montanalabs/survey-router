...
Untrusted input is a response; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a response
- **Closed type:** `type Decision = RouteSurvey(SurveyKind) | DropSurvey` where `SurveyKind` is closed
- **Sink / capability:** `grant routeSurvey` → `privileged { routeSurvey(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/survey-router/demo.sh
```
...
- `survey-router_safe.eide` — the correct design.
- `survey-router_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
